return function()
    ...
    vim.api.nvim_set_keymap('n', 'gb', "<cmd>lua require('telescope.builtin').buffers{ show_all_buffers = true }<cr>", opts)
    vim.api.nvim_set_keymap('n', 'gr', "<cmd>lua require'telescope.builtin'.lsp_references{ shorten_path = true }<CR>", opts)
    ...
    vim.api.nvim_set_keymap('n', 'gc', "<cmd>lua require'telescope.builtin'.git_branches{}<CR>", opts)
